...
class DayliController extends Controller
{
    ...
    public function create (Request $request){
        ...
        $date = Carbon::instance($DateTime)->addHours(23)->addMinutes(59)->addSeconds(59); //Verifica que la fecha actual no sea menor 
        if ($date >= Carbon::now()) {
        ...
        return $this->validateDayli($expiration , $tarifa , $tiporeserva , $plazadisponible , $activate_reserve , $id_reservetype); // Retorna a ReservationController
}else {
        return redirect()->back()->with('status', 'La Fecha ingresada es incorrecta');
         ...
         public function validateDayli ($expiration , $tiporeserva , $tarifa ,$plazadisponible,  $activate_reserve , $id_reservetype){
