...
class MonthlyController extends Controller
{
  ...
  public function create (Request $request){
    ...
    foreach ($request->input('plaza') as $array2 ) {
        ...
        $expiration =  Carbon::instance($DateTime)->addMonths(1)->addHours(21);//Tiempo limite de la expiracion 9:00 pm
        dd($expiration);
        $tarifa = Tariff::where('id_tariff', 3)->first()->id_tariff;   // Tipo de Tarifa
        $tiporeserva = ReserveType::where('id_reservetype', 3)->first()->id_reservetype; 
